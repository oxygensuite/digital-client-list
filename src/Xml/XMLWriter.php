...
use DOMException;
use DOMNode;
use OxygenSuite\OxygenDcl\Models\Model;
...
abstract class XMLWriter
{
     ...
     * @throws DOMException
     */
    protected function buildType(DOMNode $parent, string $nodeName, Model $nodeValue): DOMNode
    {
    ...
    protected function getNamespace(?string $key): ?string
    {
        ...
        if (str_contains($key, '.')) {
            ...
            // If child exists, return the corresponding namespace URI
            if ($child) {
     ...
     * @return string
     */
    abstract public function asXml($data): string;
}
